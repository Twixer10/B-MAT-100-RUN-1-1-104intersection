...
public class Sphere {
	...
	public Sphere(int mX, int mY, int mZ, int vX, int vY, int vZ, int radius) {
		super();
		this.mX = mX;
		this.mY = mY;
		this.mZ = mZ;
	...
	public void init() {
		System.out.printf("Sphere of radius %.0f", this.radius);
		...
		double a = Math.pow(this.vX, 2) + Math.pow(this.vY, 2) + Math.pow(this.vZ, 2);
		double b = (2 * ((this.vX * this.mX) + (this.vY * this.mY) + (this.vZ * this.mZ)));
		double c = (Math.pow(this.mX, 2) + Math.pow(this.mY, 2) + Math.pow(this.mZ, 2)) - Math.pow(radius, 2);
